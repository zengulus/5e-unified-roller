...
const syncYEvidenceNoteRecord = (record, note) => {
    ...
    setYScalar(record, 'id', toTrimmedString(source.id, '', 120).trim());
    setYScalar(record, 'shape', toTrimmedString(source.shape, 'zone', 20).trim() || 'zone');
...
const serializeYEvidenceNoteRecord = (record, noteId) => ({
    ...
    shape: toTrimmedString(record.get('shape'), 'zone', 20),
...
const patchYEvidenceNoteRecord = (record, baseNote = {}, nextNote = {}) => {
    ...
    mutated = patchYScalar(record, 'shape', baseNote.shape || 'zone', nextNote.shape || 'zone') || mutated;
...
class VTTCollabSession {
        ...
        if (this.canLoadColdSnapshot && cloudRow && cloudRow.ok && cloudRow.snapshot) {
            ...
            this.updateStatus({
                ...
                detail: cloudRow && !cloudRow.ok
                    ? (cloudRow.error || 'PostgreSQL VTT snapshot could not be loaded.')
                    : 'No PostgreSQL VTT snapshot exists for this room yet.',
    ...
    computeLiveDetail() {
        ...
        const hasRemoteRoomSaver = Array.from(this.remotePresence.values()).some((peer) => peer && peer.canSaveRoom);
        if (state === 'live') {
        ...
        if (hasRemoteRoomSaver && this.transportConnected && !this.renderRoomSeeded) return 'DM is live; confirming VTT document sync...';
    ...
    scheduleCloudFlush(options = {}) {
        ...
        if (!this.canCheckpointRoom(opts)) {
            if (this.pendingFlushTimer) {
                clearTimeout(this.pendingFlushTimer);
                this.pendingFlushTimer = null;
            }
        ...
        this.pendingFlushTimer = setTimeout(() => {
            const pendingOpts = this.pendingFlushOptions && typeof this.pendingFlushOptions === 'object'
                ? { ...this.pendingFlushOptions }
                : {};
            if (!this.canCheckpointRoom(pendingOpts)) {
                this.pendingReadyFlush = !!this.isDirty;
                return;
            }
            if (!this.isCloudSaveLeader()) {
    ...
    canCheckpointRoom(options = {}) {
        const opts = options && typeof options === 'object' ? options : {};
        if (opts.forceOverwrite) return true;
        if (typeof this.options.canSaveRoom !== 'function') return true;
        try {
            return !!this.options.canSaveRoom();
        } catch (err) {
            return false;
        }
    }

    handleSavePermissionChanged() {
        if (this.pendingFlushTimer) {
            clearTimeout(this.pendingFlushTimer);
            this.pendingFlushTimer = null;
        }
        if (this.isDirty || this.pendingReadyFlush) {
            this.scheduleCloudFlush({
                ...(this.pendingFlushOptions && typeof this.pendingFlushOptions === 'object' ? this.pendingFlushOptions : {})
            });
        }
    ...
    async forceAuthoritativeSnapshot(payload, options = {}) {
        ...
        if (!this.canCheckpointRoom()) {
            return {
                ...
                error: 'Only the DM can force the VTT room snapshot.'
            };
        }
