...
- **Data & Creator** – `card-io` handles import/export/reset plus Discord toggles. The creator overlay walks you through race/class ability assignments, auto-fills skill proficiencies, and seeds the base sheet, while the level-up workflow guides HP, hit dice, skill, feature, and caster progression for the active character.
...
## Table Tips
- Use the guided `Level up selected character` workflow for progression so HP, hit dice, skills, features, and caster settings stay in sync.
