...
- **Board Recovery Panel (Secret Mode)** – Inspect the live Campaign/Case board room, restore recent snapshots, promote this browser's mirrored board to live, bust a corrupted room, clear stale browser board caches before reseeding, clear only local Board/VTT storage caches when browser quota is full, and run `Sync Linked Timeline Events` to rebuild clue-linked timeline event notes/deeplinks from persisted board nodes.
...
- Import/export here touches the unified `RTF_STORE` stack (Hub, Campaign Board, Case Board, Dashboard, Roster, Locations, Requisitions, Campaign Timeline, Case Timeline, Ledger, Encounters, HQ, and VTT). Standalone utilities like the Character Sheet and Narrative Engine keep their own local saves. Session Tracker still keeps local presets/logs, but its combat mirror can now pull canonical initiative from the active case VTT.
