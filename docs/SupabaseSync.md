...
Use `Pull Latest` and `Push Now` for manual control. Non-Yjs campaign pages show `Editing` while local edits are pending, then auto-sync after 8 seconds idle or 30 seconds active. Board/VTT collaboration uses Yjs for live traffic; Supabase room writes are slower checkpoints for restoring state between sessions, with final flushes on hide/unload/manual save.
